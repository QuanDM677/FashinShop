...
import fashion.store.ui.controller.HistoryController;
...
public class HistoryJDialog extends javax.swing.JDialog implements HistoryController {
    ...
    private void initComponents() {
        ...
        tblBills.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã phiếu", "Khách hàng", "Thời điểm tạo phiếu", "Thời điểm thanh toán", "Trạng thái", "Tổng tiền", "Tiền giảm", "Thành tiền sau giảm"
            }
        ) {
    ...
    public void fillBills() {
        ...
        for (Bill bill : bills) {
            var customer = new fashion.store.dao.impl.CustomerDAOImpl().findById(bill.getCustomerId());
            var details = new fashion.store.dao.impl.BillDetailDAOImpl().findByBillId(bill.getId());

            double totalGoc = 0;
            for (var d : details) {
                totalGoc += d.getUnitPrice() * d.getQuantity();
            }

            double tongThanhTien = 0;
            for (var d : details) {
                tongThanhTien += d.getTotalPrice();
            }

            double giamGiaKhachHang = 0;
            if (customer != null && customer.isEnabled()) {
                giamGiaKhachHang += tongThanhTien * 0.05;
                if (tongThanhTien >= 500_000) {
                    giamGiaKhachHang += tongThanhTien * 0.10;
                }
            } else if (tongThanhTien >= 500_000) {
                giamGiaKhachHang += tongThanhTien * 0.10;
            }

            double thanhTienSauGiam = tongThanhTien - giamGiaKhachHang;

            model.addRow(new Object[]{
                bill.getId(),
                (customer != null) ? customer.getName() : "Vãng lai",
                XDate.format(bill.getCheckin(), "dd-MM-yyyy HH:mm"),
                XDate.format(bill.getCheckout(), "dd-MM-yyyy HH:mm"),
                getStatusName(bill.getStatus()),
                String.format("%.0f", totalGoc),
                String.format("-%.0f", giamGiaKhachHang),
                String.format("%.0f", thanhTienSauGiam)
            });
        }
    }
